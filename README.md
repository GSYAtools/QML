# Effective Depth in Quantum Machine Learning Algorithms: Gradient Attenuation in VQAs under NISQ Noise

------------------------------------------------------------------------

## Overview

This repository contains the full experimental artifact accompanying the article:

> **Effective Depth in Quantum Machine Learning Algorithms:
> Gradient Attenuation in VQAs under NISQ Noise**

The goal of the repository is to:

- Provide full reproducibility of the experimental pipeline.
- Make explicit the structural assumptions behind the effective attenuation model.
- Expose the computational workflow used to estimate the effective attenuation rate ( `\epsilon`_{`\text{eff}`{=tex}} ).
- Extend and clarify the experimental artifact beyond what is described in the paper.
...
- K = number of final epochs used to compute the gradient variance proxy
...
```bash
nohup python3 qml_cybernetic_experiment.py     --n_qubits 6     --n_layers 5     --epochs 60     --shots 60     --lr 0.05     --seeds 0 1 2 3 4 5     --out_prefix est_model_q6_l5_s60_heavy     --n_samples 512     --data_noise 0.05     --test_size 0.25     --noise_kind heavy &
```
...
```bash
python collect_results_from_dirs_with_stats.py     --root_dir .     --outdir results_summary_round5     --last_k 10
```

Important parameter:

--last_k defines K in V̄^{(K)}_∇, the structural gradient variance proxy used throughout the paper.
...
```bash
python analyze_experiment_regimes_batch.py     --csv results_summary_round5/combined_per_seed.csv     --outdir results_analysis_round5_heavy     --filter_noise heavy     --n_boot 5000
```
...
```bash
python3 estimator.py     --input results_analysis_round5_heavy/
```
